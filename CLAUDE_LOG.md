...
real, ACL, altavoz, mute) se integra en `cachoporp`. Conflicto real al mergear (no cosmetico):
`cachoporp` tenia una version minima portada a mano de `server/module/phone.lua` (154 lineas, solo
el fix de ACL + `removeCallCheck` nil-safe sobre el `phone.lua` viejo basado en Mumble) mientras
`crp-experimental` tiene la reescritura completa sobre canales nativos (331 lineas, superset real
-- incluye ese mismo fix ya integrado en el nuevo modulo). Resuelto tomando la version de
`crp-experimental` entera para `server/module/phone.lua` y `CLAUDE_LOG.md` (el propio changelog,
mas completo en esa rama). El resto de archivos (native_channels.lua, native_proximity.lua,
voice_native_test.lua, VOZ.md, y los cambios en radio.lua/proximity.lua/main.lua/events.lua/
commands.lua/shared.lua/fxmanifest.lua) fusionaron limpios, sin conflicto.

`voice_useNativeCalls` sigue en `0` por defecto en `cachoporp` igual que en `crp-experimental` --
el `server.cfg` de produccion no se toca con este merge, solo el codigo. Activar la convar en
produccion es una decision aparte, todavia no tomada.
